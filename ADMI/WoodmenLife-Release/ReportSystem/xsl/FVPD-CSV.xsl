...
<xsl:stylesheet version="1.0"
	...
	<!-- Support FVPD CSV XSL Transformation Process -->
	...
	<xsl:template match="/">
  	...
  	<xsl:template match="XML__ReportExtract">  	
		<xsl:apply-templates select="reportExtractSet/REPT__FVPD__Extract" />
	</xsl:template>
  	
	<xsl:template match="REPT__FVPD__Extract">
		...
		<xsl:if test="count(detailLines) > '0'">
			...
			<!-- ,Policy Number,Cov No,Sub Cov,Number of Units,ProRata Dividend Amount,Next Year Dividend Amount -->
			<xsl:text>,Policy,Cov,Sub,Number,ProRata,Next Year&#10;</xsl:text>
			<xsl:text>,Number,No,Cov,of Units,Dividend Amount,Dividend Amount</xsl:text>
			
			<xsl:apply-templates select="detailLines/REPT__FVPD__DetailLine" />
		...
		<xsl:apply-templates select="fvpdTotals" />
	...
	<xsl:template match="REPT__FVPD__DetailLine">
		<xsl:text>&#10;</xsl:text>
		<xsl:text>,</xsl:text>
		...
		<xsl:value-of select="format-number(proRataDivAmt, '########0.00')" /><xsl:text>,</xsl:text>
		<xsl:value-of select="format-number(nextYearDivAmt, '########0.00')" />
	</xsl:template>
	
	<xsl:template match="fvpdTotals">
