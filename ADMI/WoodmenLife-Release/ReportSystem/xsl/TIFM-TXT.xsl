...
<xsl:stylesheet version="1.0"
	...
	<!-- Support TIFM IRS 1220 Format CSV XSL Transformation Process -->
 	<xsl:template match="/">
	...
	<xsl:template match="txxxEntries">
		<xsl:apply-templates select="com.lidp.model.admin.persistence.REPT__TIFM__Entry"/>
	</xsl:template>
	<xsl:template match="com.lidp.model.admin.persistence.REPT__TIFM__Entry">
        ...
        <xsl:variable name="blank545-546" select="$blank2"/>
        <xsl:variable name="blank601-662" select="concat($blank30,$blank30,$blank2)"/>
        ...
        <xsl:value-of select="$blank545-546"/>
		<xsl:value-of select="./foreignCountryOrUsPossession"/>
        <xsl:value-of select="./cusipNumber"/>
		<xsl:value-of select="./fatcaFilingIndicator"/>
	    <xsl:value-of select="$blank601-662"/>
