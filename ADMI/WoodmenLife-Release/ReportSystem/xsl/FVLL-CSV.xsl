...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="XML__ReportExtract">
        <xsl:apply-templates select="reportExtractSet/REPT__LoanValue__Extract"/>
    </xsl:template>

    <xsl:template match="REPT__LoanValue__Extract">
        ...
        <xsl:if test="count(detailLines) > '0'">
            <xsl:text>,Policy,Cov,Sub,LL Type,Rate Code,Calc Type,Interest PD TO DATE,Accrued Interest, Refund Interest, Principal Amount, lineOfBusiness, planCode, planCodeSub  &#10;</xsl:text>

            <xsl:apply-templates select="detailLines/REPT__LoanValue__DetailLine"/>
    ...
    <xsl:template match="REPT__LoanValue__DetailLine">
        <xsl:text>&#10;</xsl:text>
        <xsl:text>,</xsl:text>
        ...
        <xsl:value-of select="llType"/><xsl:text>,</xsl:text>
        <xsl:value-of select="rateCode"/><xsl:text>,</xsl:text>
        <xsl:value-of select="calcType"/><xsl:text>,</xsl:text>
        <xsl:value-of select="interestPaidToDate"/><xsl:text>,</xsl:text>
        <xsl:value-of select="format-number(accruedInterest, '########0.00')"/><xsl:text>,</xsl:text>
        <xsl:value-of select="format-number(refundInterest, '########0.00')"/><xsl:text>,</xsl:text>
        ...
        <xsl:value-of select="planCodeSub"/><xsl:text></xsl:text>
    </xsl:template>
</xsl:stylesheet>