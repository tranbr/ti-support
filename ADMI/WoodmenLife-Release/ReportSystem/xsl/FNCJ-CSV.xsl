...
<xsl:stylesheet version="1.0"
	...
	<!-- Support PRCJ CSV XSL Transformation Process -->
	<xsl:template match="/">
    	<xsl:apply-templates select="XML__ReportExtract" />
    	<xsl:apply-templates select="PMBM__CompareSegmentExt"/>
  	</xsl:template>	
	...
	<xsl:template match="reportExtractSet">
		...
		<xsl:text>Service Center,</xsl:text>
		<xsl:text>Service Center Situation Code,</xsl:text>
		<xsl:text>Service Rep,</xsl:text>
