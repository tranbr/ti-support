...
<xsl:stylesheet version="1.0"
	...
	<!-- Support WJER CSV XSL Transformation Process -->
	...
	<xsl:template match="/">
  	...
  	<xsl:template match="XML__ReportExtract">  	
		<xsl:apply-templates select="reportExtractSet/REPT__WJER__Extract" />
	</xsl:template>
  	
	<xsl:template match="REPT__WJER__Extract">
		<!-- - - - - - - - - - - - - - - - - - - --->
		...
		<!-- - - - - - - - - - - - - - - - - - - - - -->
		<!-- Print the Request Number and Basis Type -->
		<!-- - - - - - - - - - - - - - - - - - - - - -->
		...
		<xsl:text>,Policy Number,Process Control,Coverage Number,Sub Coverage,Plan Code,Plan Sub,Coverage Type,Number of Units,Issue Date,Issue Age,Coverage Status,Sex Code,Premium Per Unit,Surrender Value,Error Connector,Error Code</xsl:text>
		<xsl:text>&#10;</xsl:text>
		
		<xsl:apply-templates select="detailLines/REPT__WJER__DetailLine" />

	</xsl:template>
	
	<xsl:template match="REPT__WJER__DetailLine">
		<xsl:text>&#10;</xsl:text>
		<xsl:text>,</xsl:text>
		...
		<xsl:value-of select="coverageStatus" /><xsl:text>,</xsl:text>
		<xsl:value-of select="sexCode" /><xsl:text>,</xsl:text>
		<xsl:value-of select="format-number(premiumPerUnit, '###,###,###.000000')" /><xsl:text>,</xsl:text>
		<xsl:value-of select="format-number(aaSurrenderValue, '###,###,###.00')" /><xsl:text>,</xsl:text>
