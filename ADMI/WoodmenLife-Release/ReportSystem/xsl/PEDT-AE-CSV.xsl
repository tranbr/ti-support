...
<xsl:stylesheet version="1.0"
	...
	<!-- Support AE Section for PEDT CSV XSL Transformation Process -->
	<xsl:template match="/">
    ...
    <xsl:template name="plae">  
		...
		<xsl:text>&#10;</xsl:text>  		
 		<xsl:apply-templates select="__PFAE__EntrySet" />
    </xsl:template>		
    <xsl:template match="PFAE__Entry">	
		<xsl:text>"</xsl:text>
