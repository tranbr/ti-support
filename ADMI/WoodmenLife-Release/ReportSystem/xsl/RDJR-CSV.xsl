...
<xsl:stylesheet version="1.0"
	...
	<!-- Support RDJR CSV XSL Transformation Process --> 
 	<xsl:template match="/">
	...
	<xsl:template match="reportExtractSet">
		...
		<xsl:text>&#10;</xsl:text>
		<xsl:apply-templates select="CompDailyAccountJournal__ReportMaster" />
	</xsl:template>
	<xsl:template match="CompDailyAccountJournal__ReportMaster">
  ...
  <xsl:template match="totals">
		...
		<xsl:text>&#10;</xsl:text>
		<xsl:text>&#10;</xsl:text>
        <xsl:text>"RDJR Reoprt Totals:" </xsl:text>
